...
class getJointStates:
    ...
    def callback(self,data):
        ...
        self.joint_position = (data.position[11],) + data.position[0:7]
...
class LearningModel():
    ...
    def step(self, weights):
        ...
        self.gg.weights = self.learning_weights # Set weights to obtain their correspond joints

        plan_resp = self.applyAction() # Plan formed by joints
    ...
    def applyReset(self):
        self.gg.loadGestureFromBagJointStates("movement.bag", self.joint_names, True)
        #state = getJointStates()
        #initial_pose = state.joint_position
        initial_pose = [0.33298205586157925, -4.7589775664214073e-05, -0.021233773215461937, 0.00041857553360458155, 0.007812366709138985, -0.00022604317547436636, 0.018577739002537008, 0.00013233464147077711]
        goal_pose = [0, 0.16471575617593182, 0.8884589872474873, -1.6132110770646424, 1.4834595266009387, -1.2186207169225822, 1.3679727677743694, -1.3353656179158602]
        ...
        rospy.loginfo("Publishing initial weights...")
    ...
    def applyEndPosition(self):
        ...
        for i in range(0, len(self.plan_resp.plan.points)):
            diction.append({'positions': self.plan_resp.plan.points[i].positions, 'time_from_start':self.plan_resp.plan.times[i]})
